...
//            attributeString.addAttributes([NSAttributedString.Key.font:UIFont.boldSystemFont(ofSize: 15)], range: NSRange(location: 0, length: header.characters.count))
...
//        if appIconName.characters.count != 0 {
    ...
    func addSeprator(_ toObject:AnyObject) -> UIView{
        if #available(iOS 12.0, *) {
            ...
            let seprator = UIView(frame: CGRect(x: 0,y: (frame?.height ?? 0) + 3, width: (APP_DELEGATE.keyWindow?.frame.size.width)! - 20,height: 0.5))
